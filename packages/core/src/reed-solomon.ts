...
export async function reedSolomonFromResponse(source: Response | Promise<Response>): Promise<ReedSolomonCodec> {
  const { instance } = await WebAssembly.instantiateStreaming(await source); const exports = instance.exports as WasmExports; const memory = () => new Uint8Array(exports.memory.buffer);
  return {
    encode(shards, dataShards, parityShards) { const pointer = exports.__wbindgen_malloc(shards.length); memory().set(shards, pointer); const shardSize = shards.length / (dataShards + parityShards); const result = exports.encode(pointer, shards.length, dataShards, parityShards); if (result === 0) shards.set(memory().subarray(pointer + shardSize * dataShards, pointer + shards.length), shardSize * dataShards); exports.__wbindgen_free(pointer, shards.length); return result; },
    reconstruct(shards, dataShards, parityShards, available) { const pointer = exports.__wbindgen_malloc(shards.length); memory().set(shards, pointer); const flagsPointer = exports.__wbindgen_malloc(available.length); memory().set(available.map((value) => value ? 1 : 0), flagsPointer); const result = exports.reconstruct(pointer, shards.length, dataShards, parityShards, flagsPointer, available.length); if (result === 0) shards.set(memory().subarray(pointer, pointer + shards.length / (dataShards + parityShards) * dataShards)); exports.__wbindgen_free(pointer, shards.length); exports.__wbindgen_free(flagsPointer, available.length); return result; },
  };
}
