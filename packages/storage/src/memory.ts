...
export class MemoryShardTransport implements ShardTransport {
  ...
  async getShard(nodeId: string, objectId: string) { this.assertOnline(nodeId); const bytes = this.objects.get(`${nodeId}:${objectId}`); if (!bytes) throw new Error(`Shard not found on ${nodeId}`); return bytes.slice(); }
