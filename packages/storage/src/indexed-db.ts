...
export class IndexedDbShardTransport implements ShardTransport {
  ...
  async getShard(nodeId: string, objectId: string, signal?: AbortSignal) { signal?.throwIfAborted(); this.assertOnline(nodeId); const record = await this.transaction<{ key: string; bytes: Uint8Array } | undefined>("readonly", (store) => store.get(`${nodeId}:${objectId}`)); signal?.throwIfAborted(); if (!record) throw new Error(`Shard not found on ${nodeId}`); return new Uint8Array(record.bytes); }
