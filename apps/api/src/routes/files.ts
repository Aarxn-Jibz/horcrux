...
router.post("/:id/complete", async (c) => {
  ...
  statements.push(c.env.DB.prepare("UPDATE files SET compressed_size=?,encrypted_size=?,ciphertext_hash=?,encryption_iv=?,rs_shard_size=?,chunk_size=?,chunk_count=?,nonce_prefix=?,status='available',completed_at=datetime('now') WHERE id=?").bind(input.compressedSize ?? null, input.encryptedSize ?? null, input.ciphertextHash ?? null, input.encryptionIv ?? null, input.shardSize ?? null, input.chunkSize ?? null, input.chunkCount ?? null, input.noncePrefix ?? null, file.id));
