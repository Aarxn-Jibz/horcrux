...
router.post("/refresh", async (c) => { const token = getCookie(c, REFRESH_COOKIE); if (!token) throw new ApiError(401, "refresh_required", "Refresh session is missing"); const row = await c.env.DB.prepare("SELECT rt.id,rt.user_id,u.email,rt.expires_at,rt.revoked_at FROM refresh_tokens rt JOIN users u ON u.id=rt.user_id WHERE rt.token_hash=?").bind(await hashToken(token)).first<RefreshRow>(); if (!row || row.revoked_at || Date.parse(row.expires_at) <= Date.now()) { deleteCookie(c, REFRESH_COOKIE, cookieOptions(c)); throw new ApiError(401, "invalid_refresh", "Refresh session is invalid or expired"); } return c.json(await setSession(c, { id: row.user_id, email: row.email }, row.id)); });
router.post("/logout", async (c) => { const token = getCookie(c, REFRESH_COOKIE); if (token) await c.env.DB.prepare("UPDATE refresh_tokens SET revoked_at=datetime('now') WHERE token_hash=? AND revoked_at IS NULL").bind(await hashToken(token)).run(); deleteCookie(c, REFRESH_COOKIE, cookieOptions(c)); return c.body(null, 204); });
