...
export async function issueIceServers(env: Env, fetcher: Fetcher = fetch): Promise<IceServer[]> {
  ...
  const url = env.TURN_CREDENTIALS_URL ?? `https://rtc.live.cloudflare.com/v1/turn/keys/${encodeURIComponent(env.TURN_KEY_ID)}/credentials/generate`;
  const response = await fetcher(url, { method: "POST", headers: { Authorization: `Bearer ${env.TURN_API_TOKEN}` } });
