...
export function Landing({ user, onEnter, onAuth }: { user: User | null; onEnter(): void; onAuth(): void }) {
  return <main className="landing">
    ...
    <section className="landing-hero">
      ...
      <div className="artifact" aria-label="Five fragments separate, two disappear, and three rebuild the artifact"><Suspense fallback={null}><LazyFragmentHero /></Suspense><div className="artifact-core">H</div>{["I", "II", "III", "IV", "V"].map((label, index) => <i key={label} className={`fragment fragment-${index + 1}`}>{label}</i>)}<p>five fragments · two vanish · three restore</p></div>
    </section>
...
const LazyFragmentHero = lazy(() => import("./FragmentHero").then((module) => ({ default: module.FragmentHero })));
