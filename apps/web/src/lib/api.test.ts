...
describe("API refresh coordination", () => {
  test("uses one refresh for simultaneous expired requests", async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      if (path === "/auth/challenge") return Response.json({ kdf: { version: "pbkdf2-sha256-v1", algorithm: "PBKDF2", hash: "SHA-256", iterations: 310000, salt: "AAAAAAAAAAAAAAAAAAAAAA==", derivedKeyLength: 256 } });
