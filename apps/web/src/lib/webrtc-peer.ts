...
export class HorcruxPeer {
  ...
  static async connect(nodeId: string, iceServers: RTCIceServer[] = []) {
    ...
    const offer = await connection.createOffer(); await connection.setLocalDescription(offer); await sendWebRtcSignal(session.sessionId, { nodeId, type: "offer", payload: offer.sdp! });
