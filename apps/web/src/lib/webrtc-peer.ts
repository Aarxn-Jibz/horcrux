...
export class HorcruxPeer {
  ...
  static async connect(nodeId: string) {
    ...
    try {
      const offer = await connection.createOffer(); await connection.setLocalDescription(offer); await sendWebRtcSignal(session.sessionId, { nodeId, type: "offer", payload: connection.localDescription!.sdp! });
      await peer.waitForAnswer(session.sessionId, nodeId); await opened; return peer;
    } catch (error) { peer.close(); throw error; }
  }
  private async waitForAnswer(sessionId: string, nodeId: string) { const deadline = Date.now() + 30_000; const candidates: string[] = []; const seen = new Set<string>(); while (!this.closed && Date.now() < deadline) { const response = await readWebRtcSignals(sessionId); for (const signal of response.signals) { if (seen.has(signal.payload)) continue; seen.add(signal.payload); if (signal.type === "answer" && !this.connection.currentRemoteDescription) await this.connection.setRemoteDescription({ type: "answer", sdp: signal.payload }); else if (signal.type === "ice-candidate") candidates.push(signal.payload); } if (this.connection.currentRemoteDescription) for (const candidate of candidates.splice(0)) await this.connection.addIceCandidate(JSON.parse(candidate)); if (this.channel.readyState === "open") return; await new Promise((resolve) => setTimeout(resolve, 250)); } throw new Error(`WebRTC answer timed out for ${nodeId}`); }
